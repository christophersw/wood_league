...
## Implementation deviations (recorded during subagent-driven execution)

The code is the source of truth; the spec (`2026-05-18-vast-provisioning-design.md`) was updated to match. Plan-vs-implemented differences:

1. **T4 test** — drafted test imported `import logging` (unused → ruff F401). Removed; the test uses `self.assertLogs`. Also added 2 extra parity tests (`list_instances` non-2xx/network → `[]`) and a call-site comment confirming `POST /api/v0/bundles/` is the documented vast search endpoint.
2. **T6** — the single `_reap` exceeded the project's cyclomatic-complexity hard gate (grade D, cc 21). Decomposed into `_reap_decision`/`_reap_one`/`_orphan_vast_id`/`_reap_orphans`/`_reap` (all ≤ grade B), behavior-preserving. Also added a `not inst.launch_worker_ids` guard to the never-registered condition (required for `test_pre_launch_worker_not_bound`; sound — an instance launched into a non-empty-worker environment is reaped only via `hard_deadline`/drained, not never-registered).
3. **T7** — drafted test imported `from django.utils import timezone` (unused → ruff F401). Removed. **Behavior refinement:** the `launching` `AnalysisInstance` row is now created *after* a successful offer search (still before the billable create), not before the search. On `NoVastOfferError` nothing is recorded (no box exists to recover), so the schedule stays `pending` with no accumulating dead rows. Test renamed `test_no_offer_creates_no_instance_schedule_stays_pending`.
4. **T8** — drafted test used semicolon-joined statements (ruff E702) and a physically-impossible time model (worker `last_seen` before `launched_at`). Rewritten with realistic simulated tick timing (backdated `launched_at` −40 min; worker `last_seen` −20 min, after launch but past the 15-min stale cutoff) and a strict `destroy must not be called on a launch-only tick` guard on Tick 1. Production code was NOT changed for T8.
