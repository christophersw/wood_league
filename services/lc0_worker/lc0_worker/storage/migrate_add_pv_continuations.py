...
Description:
    Adds pv_san_1, pv_san_2, pv_san_3 Text columns to the lc0_move_analysis table
    to store full principal variation continuations from Lc0 analysis.
...
def migrate_add_pv_continuations(engine) -> None:
    """
    Add pv_san_1, pv_san_2, pv_san_3 nullable Text columns to lc0_move_analysis table.
    ...
    Side effects:
        Alters the lc0_move_analysis table schema by adding three columns if they
    ...
    with engine.begin() as connection:
        if is_postgresql:
            for column_name in column_definitions:
                sql = f"""
                    ALTER TABLE lc0_move_analysis
                    ADD COLUMN IF NOT EXISTS {column_name} TEXT
                """
                try:
                    connection.execute(sa_text(sql))
                    log.info(f"Added column {column_name} to lc0_move_analysis (PostgreSQL)")
                except OperationalError as exc:
                    log.warning(
                        f"Failed to add column {column_name} to lc0_move_analysis: {exc}"
                    )
        else:
            # SQLite: no IF NOT EXISTS support for ALTER TABLE, use try/except
            for column_name in column_definitions:
                sql = f"ALTER TABLE lc0_move_analysis ADD COLUMN {column_name} TEXT"
                try:
                    connection.execute(sa_text(sql))
                    log.info(f"Added column {column_name} to lc0_move_analysis (SQLite)")
                except OperationalError as exc:
                    # Column likely already exists
                    if "duplicate column" in str(exc).lower():
                        log.info(
                            f"Column {column_name} already exists in lc0_move_analysis (SQLite)"
                        )
                    else:
                        log.warning(
                            f"Failed to add column {column_name} to lc0_move_analysis: {exc}"
